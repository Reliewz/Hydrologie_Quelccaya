...
# Date: 2026.07.07
# Outputs:
  # list with a data frame and small report on how many values have been detected. The data frame can then be used for further flagging workflows.
#======================================================================
...
#' @param date_column Character string. Column which contains the temporal information for the time series. Used for report. Default: "Date".
#' @param measurement_columns Character vector containing all columns where measurement values are stored.
...
#' Whole hydrological sensors or meteorological stations can be tested by providing a shared identification ID describing the whole sensor groups.
...
#' \code{\link{function_log_qc_decisions}} to generate a report
...
qc_completeness_test <- function(df,
  ...
  if (!all(measurement_columns %in% names(df))) {
